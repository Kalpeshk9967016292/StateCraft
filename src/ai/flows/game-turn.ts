...
3.  **Check for Game Over Scenarios**: After updating stats, check if a game-ending condition is met. A political coup (vote of no confidence) must be an **EXTREMELY RARE** and dramatic event. **Do not trigger this on early turns (e.g., before turn 4 or 5).** It should only be triggered as a result of *sustained, long-term failure* (e.g., 3-4 consecutive turns of terrible stats). For a coup to be plausible, Opposition Strength must be extremely high (e.g., > 90), Public Approval must be critically low (e.g., < 20), and other factors like Economic Health must also be in a poor state for a prolonged period. The player's government should be on the brink of collapse from multiple angles. If this very high bar is met, set 'isGameOver' to true and provide a compelling 'gameOverReason'. Otherwise, 'isGameOver' must be false.
