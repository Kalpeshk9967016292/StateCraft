...
3.  **Check for Game Over Scenarios**: After updating stats, check if a game-ending condition is met. One key scenario is a political coup (vote of no confidence). This must be an EXTREMELY RARE and dramatic event. **Do not trigger this based on one bad turn.** It should only be triggered as a result of sustained, long-term failure (e.g., 3-4 consecutive turns of terrible stats). For a coup to be plausible, Opposition Strength must be extremely high (e.g., > 90), Public Approval must be critically low (e.g., < 20), and other factors like Economic Health must also be in a poor state for a prolonged period. The player's government should be on the brink of collapse from multiple angles. If this very high bar is met, set 'isGameOver' to true and provide a compelling 'gameOverReason'.
