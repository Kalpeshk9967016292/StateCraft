...
3.  **Check for Game Over Scenarios**: After updating stats, check if a game-ending condition is met. One key scenario is a political coup (vote of no confidence). This must be a RARE and dramatic event. Only trigger this in the most extreme circumstances where the government is clearly failing on all fronts. For a coup to be plausible, Opposition Strength must be extremely high (e.g., > 90), Public Approval must be critically low (e.g., < 20), and other factors like Economic Health should also be in a poor state. It should be a culmination of sustained poor performance. If such an event occurs, set 'isGameOver' to true and provide a compelling 'gameOverReason'.
